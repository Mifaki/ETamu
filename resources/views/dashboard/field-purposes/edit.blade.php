...
@section('title', 'Edit Tujuan Bidang - Dashboard')
...
@section('content')
    <div class="flex items-center justify-center h-full">
        <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md max-w-lg w-full">
            ...
            <form action="{{ route('dashboard.field-purposes.update', $fieldPurpose) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        value="{{ old('name', $fieldPurpose->name) }}" required>
                    @error('name')
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 dark:text-gray-200" for="description">
                        Deskripsi
                    </label>
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $fieldPurpose->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end">
                    <a href="{{ route('dashboard.field-purposes.index') }}"
